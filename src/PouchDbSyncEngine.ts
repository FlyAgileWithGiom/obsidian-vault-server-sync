 ...
 * the token manager's ClientId/Secret, both of which are resolved by the caller.
 */
...
export class PouchDbSyncEngine {
  ...
  constructor(
    ...
    /**
     * Optional remote PouchDB handle factory (feat/sync-gateway-oauth).
     *
    ...
    /**
     * Optional gateway credential resolver (feat/sync-gateway-oauth).
     *
    ...
    return new Promise<void>((resolve) => {
      ...
      emitter.on("complete", () => {
        ...
        if (this.started) {
          ...
          // and the resolve() call immediately below must fire regardless of gateway setup.
          void this.startLiveSync();
        }
        resolve();
      });
   ...
   *
   * Phase A/B fallback (criterion 6):
   ...
   * cast at the three call sites — the cast is warranted because our local PouchDB type
