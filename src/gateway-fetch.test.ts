...
// ---------------------------------------------------------------------------
// TokenManager — no refresh token (cannot acquire a token without one)
// ---------------------------------------------------------------------------
// Public PKCE clients have no client_secret and therefore no client_credentials
// fallback. The initial token always comes from a stored refresh_token obtained
// from the interactive Clerk login. With no refresh token, there is nothing to
// exchange — the manager throws so the caller can prompt the user to log in.

describe("TokenManager — no refresh token", () => {
  it("throws (does not hit /token) when no refresh token is stored", async () => {
    const store = fakeStore(); // no refresh token stored
    const fetchMock = mockFetchReturning([{ status: 200, body: tokenResponse() }]);
    vi.stubGlobal("fetch", fetchMock);

    const manager = makeTokenManager(testManagerOpts(store));

    await expect(manager.getValidToken()).rejects.toThrow();
    // No client_credentials fallback: the token endpoint must NOT be called.
...
describe("TokenManager — refresh_token grant", () => {
  it("uses refresh_token grant when a stored refresh token exists", async () => {
    ...
    // Public PKCE client: client_id is sent (Clerk binds the refresh token to it),
    // but there is NO client_secret to send.
