...
// ---- createStrategy fetchServerDocIds tests ----

/**
 * Tests for the fetchServerDocIds closure wired into PouchDbSyncEngine's 5th
 * constructor argument (BUG-77 / replaceLocalFromServer orphan pruning).
 *
 * The fetcher must use Obsidian's requestUrl API (not raw fetch) so that it
 * works on every platform (desktop, iOS) without CORS issues, and never embed
 * credentials in the URL (Fetch spec rejects credential-in-URL requests).
 */
describe("VaultSyncPlugin.createStrategy — fetchServerDocIds", () => {
  let requestUrlSpy: ReturnType<typeof vi.spyOn>;

  beforeEach(() => {
    vi.useFakeTimers();
    vi.mocked(SyncEngine).mockClear();
    // Spy on the requestUrl export from the obsidian mock module so we can
    // assert call arguments and inject return values.
    requestUrlSpy = vi.spyOn(obsidianMock, "requestUrl");
  });
  ...
  async function loadPluginWithSettings(settings: Partial<typeof DEFAULT_SETTINGS>): Promise<void> {
    const plugin = makePlugin();
    // Write settings to the vault adapter so loadSettings() picks them up
    // (onload() calls loadSettings() which overwrites plugin.settings).
    const merged = { ...DEFAULT_SETTINGS, couchDbName: "vault-test", ...settings };
    plugin.app.vault.adapter._setStored(VAULT_SYNC_CONFIG_FILE, JSON.stringify(merged));
    (plugin.app.vault as unknown as { getName(): string }).getName = () => "test-vault";
    (plugin as unknown as { addRibbonIcon: unknown }).addRibbonIcon = vi.fn().mockReturnValue(makeEl());
    (plugin as unknown as { addStatusBarItem: unknown }).addStatusBarItem = vi.fn().mockReturnValue(makeEl());
    plugin.app.vault.on = vi.fn().mockReturnValue({ unload: () => {} }) as unknown as typeof plugin.app.vault.on;
  ...
  it("passes fetchServerDocIds as 5th arg to PouchDbSyncEngine", async () => {
    await loadPluginWithSettings({});

    const ctorCall = vi.mocked(SyncEngine).mock.calls.at(-1)!;
    const fetcher = ctorCall[4];
    expect(typeof fetcher).toBe("function");
  });

  it("fetchServerDocIds calls requestUrl (not raw fetch) with correct URL and no credentials in URL", async () => {
    await loadPluginWithSettings({
    ...
    requestUrlSpy.mockResolvedValueOnce({
      status: 200,
      json: { rows: [{ id: "file/a.md" }, { id: "file/img.png" }] },
      text: "",
    });

    const fetcher = vi.mocked(SyncEngine).mock.calls.at(-1)![4]!;
    const ids = await fetcher();

    expect(ids).toEqual(["file/a.md", "file/img.png"]);

    expect(requestUrlSpy).toHaveBeenCalledOnce();
    const callArg = requestUrlSpy.mock.calls[0][0];
    // URL must NOT contain credentials
    expect(callArg.url).not.toContain("alice");
    expect(callArg.url).not.toContain("secret");
    // URL must point to the correct DB endpoint
    expect(callArg.url).toBe("https://sync.example.com/vault-mine/_all_docs?include_docs=false");
    // Auth must be passed as header
    expect(callArg.headers?.["Authorization"]).toMatch(/^Basic /);
  });

  it("fetchServerDocIds omits Authorization header when no credentials configured", async () => {
    await loadPluginWithSettings({
      couchDbUrl: "https://sync.example.com",
      couchDbName: "vault-mine",
      couchDbUser: "",
  ...
  it("fetchServerDocIds throws when requestUrl returns a non-200 status", async () => {
    await loadPluginWithSettings({
      couchDbUrl: "https://sync.example.com",
      couchDbName: "vault-mine",
    });
